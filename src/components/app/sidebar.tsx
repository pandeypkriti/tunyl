...
export function Sidebar({ queueCount, name }: { queueCount: number; name: string }) {
  ...
  return (
    <nav aria-label="Sections" className="flex gap-1 overflow-x-auto border-b border-[color:var(--border)] bg-white px-3 py-2 min-[900px]:sticky min-[900px]:top-14 min-[900px]:h-[calc(100vh-56px)] min-[900px]:flex-col min-[900px]:overflow-y-auto min-[900px]:border-b-0 min-[900px]:border-r min-[900px]:px-3 min-[900px]:py-4">
      {groups.map((g) => (
        <div key={g.label} className="contents min-[900px]:mb-4 min-[900px]:block">
          <div className="tag hidden px-3 pb-1.5 min-[900px]:block">{g.label}</div>
          {g.items.map((it) => {
            const on = active(it.href);
            return (
              <Link key={it.href} href={it.href} aria-current={on ? "page" : undefined}
                className={cn("flex items-center gap-2.5 whitespace-nowrap rounded-md px-3 py-2 text-[14px] transition-colors min-[900px]:w-full", on ? "bg-[color:var(--primary-soft)] font-medium text-[color:var(--primary-ink)]" : "text-[color:var(--ink-2)] hover:bg-[color:var(--surface-2)] hover:text-[color:var(--ink)]")}>
      ...
      <div className="hidden min-[900px]:mt-auto min-[900px]:block">
