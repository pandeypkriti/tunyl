...
export function Topbar({ name }: { name: string }) {
  const { open } = useAsk();
  return (
    <header className="sticky top-0 z-20 flex h-14 items-center justify-between gap-3 border-b border-[color:var(--border)] bg-white px-4 md:px-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="hidden text-[13px] text-[color:var(--ink-2)] md:inline">{name}</span>
      </div>
    </header>
  );
}
